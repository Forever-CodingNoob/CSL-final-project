...
void Scan(){
  for(int i=0;i<4;i++){
    irValuesRaw[i]=analogRead(irPins[i]);
    if(irValuesRaw[i]<threshold_low[i]) 
      irValues[i]=C_WHITE;
    else if(irValuesRaw[i]<threshold_high[i])
      irValues[i]=C_GRAY;
    else
      irValues[i]=C_BLACK;
    /*
    Serial.print(irValuesRaw[i]);
    Serial.print("(");
    if(irValues[i]==C_WHITE) Serial.print("W");
    if(irValues[i]==C_GRAY) Serial.print("G");
    if(irValues[i]==C_BLACK) Serial.print("B");
    Serial.print(")\t");
    */
  }
  //Serial.println();
...
uint8_t extractIRValueBin(int value){
  uint8_t bin = (uint8_t)0;
  for(int i=0;i<4;i++)
    bin|=(!(irValues[i] ^ value))<<(4-i-1);
  return bin;
} 

void UpdateCorrection() {
  uint8_t isGray=extractIRValueBin(C_GRAY);
  //uint8_t isWhite=extractIRValueBin(C_WHITE);

  switch(isGray){
    case 0b1111:
      //correction = 305*sgn(correction);
...
void StripeStop(){
  ...
  for(int i=0;i<4;i++){
    white_count+=!(irValues[i] ^ C_WHITE);
    black_count+=!(irValues[i] ^ C_BLACK);
  }
...
void Steer(){
  if(sgn(previous_correction)!=sgn(correction)){
